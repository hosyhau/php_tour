...
<div class="content-wrapper">
  <div class="container-fluid">
    ...
    <div class="container">
      ...
      <div class="row">
        <table class="table table-hover">
          ...
          <tbody>
          <?php $i=1; foreach ($binhluan as $key => $value): ?>
            <tr class="rename">
              <td><?= $i; ?> <input class="id_coment" style="display: none;" type="text" value="<?= $value['coment_id']; ?>"></td>
              <td><?= $value['cus_name']; ?></td>
              <td><?= $value['tour_name']; ?></td>
              <td><?= $value['content']; ?></td>
              <td><button type="button" class="btn btn-danger xoa_coment">Xóa</button></td>
            </tr>
          <?php $i++; endforeach ?>
          </tbody>
        </table>
    ...
    <script>
      // Start ajax delete_comenet
      $('table').on('click', '.xoa_coment', function(event) {
              xoa = $(this).parent().parent();
              var parentOjb = $(this).closest('.rename');
              var id = parentOjb.find('.id_coment').val();            
              $.ajax({
                  url: '<?php echo base_url(); ?>index.php/coment/xoa_coment',
                  type: 'POST',
                  dataType: 'json',
                  data: {id: id}
              })
